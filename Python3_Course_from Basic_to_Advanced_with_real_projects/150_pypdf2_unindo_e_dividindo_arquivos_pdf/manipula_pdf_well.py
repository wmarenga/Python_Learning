...
# Juntar PDFs
caminho_dos_pdfs = str(Path(__file__).parent) + '\\pdf'
print(caminho_dos_pdfs)
...
for root, dirs, files in os.walk(caminho_dos_pdfs):
    for file in files:
        ...
        arquivo_pdf = open(caminho_completo_arquivo, 'rb')  # Leitura em bites
...
with open(f'{caminho_dos_pdfs}\\novo_arquivo.pdf', 'rb') as arquivo_pdf:
    ...
    for num_pagina in range(num_paginas):
        ...
        with open(f'{Path(caminho_dos_pdfs).parent}\\novos_pdfs\\{num_pagina}.pdf', 'wb') as novo_pdf:
