...
caminho_arquivo_Excel ='D:/23) Programação/Cursos/Python/4) Python para Iniciantes (Direto ao que interessa)/Aula 57 (Lojas e produtos que mais geram receita)/caso_estudo.xlsx'
...
""" Vamos encontrar quantas vezes e maior a receita do Rio de Janeiro em relacao aos outros estados, para isso nos pegarmos o maior valor
dos dados de graf_dados (valor da receita do RJ) e dividindo pela media das receitas dos outros estados (desconsiderando o valor maximo do RJ). 
"""
...
" O primeiro e o terceiro valor serao inteiros (%i) e o segundo valor sera float com duas casas decimais (%.2f)"
print('O maior valor e %i, sendo %.2f vezes a media dos demais, que e %i' % (graf_dados.max(), graf_dados.max()/graf_dados[graf_dados!=graf_dados.max()].mean(), graf_dados[graf_dados!=graf_dados.max()].mean()))
