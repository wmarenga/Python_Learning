...
caminho_arquivo_Excel ='D:/23) Programação/Cursos/Python/4) Python para Iniciantes (Direto ao que interessa)/Aula 68 (Mapa de Calor de Inadimplencia)/caso_estudo.xlsx'
...
" Criando um filtro para as colunas que queremos trabalhar "
...
" Criando um filtro para as colunas que queremos trabalhar (produto_produto e produto_valor) e realizamos a soma na coluna produto_produto"
...
" ## ANALISE DE TEMPO (VENDAS E RECEITAS) ## "

" Tempo medio de pagamento "

print(df_juncao)

" Podemos aplicar uma media em todos os tempo de pagamento na coluna (tempo_pg)"
print(df_juncao.tempo_pg.mean())

" Para verificar esta media iremos criar um grafico (boxplot)"
df_juncao.boxplot('tempo_pg')
plt.title('Boxplot para Tempo de Pagamento')
plt.show()

" O grafico possui um distribuicao bem uniforme e sem Outliers, porem precisamos avaliar por um histograma e uma curva normal desse dado"
" Para isto vamos usar o Seaborn "
" Com o parametro (kde=True) iremos visualizar tambem a curva de densidade "
sns.histplot(data=df_juncao.tempo_pg, kde=True)
plt.title('Histograma para Tempo de Pagamento')
plt.show()

" Verificamos que o grafico nao mostra uma curva distribuicao normal, sendo assim nos nao temos uma previsibilidade, pois os dados nao estao concentrados "

" Usando o comando (describe) podemos ver varios dados (media, min, max, etc)"
print(df_juncao.tempo_pg.describe())

" ## TEMPO DE PAGAMENTO POR CIDADE E PRODUTO ## "

print(df_juncao)

" Criando um groupby da coluna (loja_cidade) e retirara a media da coluna (tempo_pg) "
print(df_juncao.groupby('lojas_cidade').mean().tempo_pg.sort_values(ascending=False))

" Criando grafico Bosxplot "
" O grafico Boxplot nao utiliza a media e sim todo o conjunto de dados, sendo assim nao podemos usar o groupby acima "
plt.figure(figsize=(7,4))
" Plotando apenas o df com as colunas (lojas_cidade e tempo_pg) "
df_juncao[['lojas_cidade', 'tempo_pg']].groupby('lojas_cidade').boxplot('tempo_pg')
plt.title('Boxplot para Tempo de Pagamento por Cidade')
plt.xticks(rotation=45)
plt.show()
""" Poderiamos concluir que o comportamento de tempo de pagamento seria muito difirente de uma cidade para outra, se as amplitudes 
fossem difirente ou os limites inferiores e/ou superiores fossem muito distintos em relacao as outras cidades. Santa Catarina tem uma
amplitute um pouco diferente, porem as variacoes sao muito pequenas. Como resultado, poderiamos concluir que nao existe um diferenca
significativa no tempo de pagamento entre as cidades. Por este motivo, nos precisaremos avaliar o histogramas para avaliar melhor. """

print(df_juncao.groupby('produto_produto').tempo_pg.mean())

" Gerando um grafico de histograma "
plt.figure(figsize=(7,4))
df_juncao[['produto_produto', 'tempo_pg']].groupby('produto_produto').boxplot('tempo_pg')
plt.title('Boxplot para Tempo de Pagamento por Produto')
plt.xticks(rotation=45)
plt.show()

""" Podemos verificar que o comportamento mostrado no grafico e bem similar ao grafico anterior (tempo de pagamento por lojas). Somente
podemos observar que a mesa possui um amplitude menor, porem com medias de pagamento bem proximas as dos outros produtos. """

" ## INADIMPLENCIA POR PRODUTO E POR LOJA ## "

" Analise de Inadimplencia "

" Inadimplencia total "
print(df_juncao.pg.mean())

" Inadimplencia por cidade "
print(df_juncao[df_juncao.pg == 0].groupby('lojas_cidade').count().produto_valor.sort_values(ascending=False))

" Plotando um grafico de barras para as lojas inadimplentes "
plt.figure(figsize=(15,5))
" Gerando um Subplot para colocar os dois graficos juntos "
plt.subplot(1,2,1)
graf_dados = df_juncao[df_juncao.pg == 0].groupby('lojas_cidade').count().produto_valor.sort_values(ascending=False)
plt.bar(graf_dados.index, graf_dados.values)
plt.title(" Inadimplencia por Lojas ")
plt.xticks(rotation=45)

" Gerando um percentual da coluna (pg)"
print(df_juncao.groupby('lojas_cidade').mean().pg.sort_values(ascending=False))

" Plotando um grafico de barras com percentual de inadimplentes por loja "
plt.subplot(1,2,2)
graf_dados = df_juncao.groupby('lojas_cidade').mean().pg.sort_values(ascending=False)
plt.bar(graf_dados.index, graf_dados.values)
plt.title(" Percentagem de Pagamentos por Loja ")
plt.xticks(rotation=45)
plt.show()
" Salvador possui um indice baixo de inadimplencia (10%), enquanto que o Rio de Janeiro possui um indice alto (60%)"

" Inadimplencia por Produto "
print(df_juncao)

" Analisando quando (pg==0) para produtos com inadimplencia "
print(df_juncao[df_juncao.pg == 0].groupby('produto_produto').count().produto_valor.sort_values(ascending=False))

" Salvar na variavel graf_dados"
graf_dados = df_juncao[df_juncao.pg == 0].groupby('produto_produto').count().produto_valor.sort_values(ascending=False)

" Plotando um grafico de barras para inadimplentes por produtos "
plt.figure(figsize=(15,5))
" Gerando o primeiro Subplot para colocar os dois graficos juntos "
plt.subplot(1,2,1)
plt.bar(graf_dados.index, graf_dados.values)
plt.title(" Inadimplencia por Produtos ")
plt.xticks(rotation=45)

" Plot com a percentagem de vendas"
" Gerando o segundo Subplot para colocar os dois graficos juntos "
plt.subplot(1,2,2)
graf_dados = df_juncao.groupby('produto_produto').mean().pg.sort_values(ascending=False)
plt.bar(graf_dados.index, graf_dados.values)
plt.title(" Percentagem de pagamentos por Produtos ")
plt.xticks(rotation=45)
plt.show()
" No primeiro grafico, notamos que o laptop basico tem a maior inadimplencia (porem vende mais)"
""" Quando avaliamos o segundo grafico, notamos que o laptop gamer tem o moner percentual de pagamento por produto, enquanto 
que o laptop basico apresenta aproximadamente 70% de pagamentos """
" O laptop gamer gera um prejuizo pois apenas 40% gera pagamento"

" Analisando a receita perdida por inadimplencia "

print(df_juncao[df_juncao.pg == 0].groupby('produto_produto').sum().produto_valor.sort_values(ascending=False))
" Salvar na variavel graf_dados"
graf_dados = df_juncao.groupby('produto_produto').sum().produto_valor.sort_values(ascending=False)
...
plt.subplot(1,2,1)
plt.bar(graf_dados.index, graf_dados.values)
plt.title(" Receita de Inadimplencia por Produtos ")
plt.xticks(rotation=45)

" Plot com a percentagem de vendas"
" Gerando o segundo Subplot para colocar os dois graficos juntos "
plt.subplot(1,2,2)
graf_dados = df_juncao[df_juncao.pg == 0].groupby('produto_produto').sum().produto_valor/df_juncao.groupby('produto_produto').sum().produto_valor.sort_values(ascending=False)
plt.bar(graf_dados.index, graf_dados.values)
plt.title(" Percentagem de receita perdida por Produtos ")
plt.xticks(rotation=45)
plt.show()

" ## Gerando um Mapa de Calor de Inadimplencia ## "
" Comparacao entre a loja_cidade e o nome do produto "
" Todo mapa de calor precisa de uma Pivot Table "

import seaborn as sns
graf_dados = df_juncao.pivot_table(index='produto_produto', columns='lojas_cidade', values='pg', aggfunc='mean')
sns.heatmap(graf_dados, annot=True)
plt.show()

import seaborn as sns
graf_dados = df_juncao.pivot_table(index='produto_produto', columns='cliente_idade', values='pg', aggfunc='mean')
sns.heatmap(graf_dados)
plt.show()

import seaborn as sns
graf_dados = df_juncao.pivot_table(index='lojas_cidade', columns='cliente_idade', values='pg', aggfunc='mean')
sns.heatmap(graf_dados)
plt.show()